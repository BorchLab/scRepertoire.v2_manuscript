...
# Make sure Spike.Specific is logical or factor for shape/aesthetic distinction
TRA.AF$Spike.Specific <- as.factor(TRA.AF$Spike.Specific)

# Plot
ggplot(TRA.AF, aes(x = PC1, y = PC2, color = Time, shape = Spike.Specific)) +
  geom_point(size = 3, alpha = 0.8) +
  scale_shape_manual(values = c(16, 17)) +  # change shape if needed
  theme_minimal() +
  facet_wrap(~Time) + 
  scale_color_viridis(option = "inferno", discrete = TRUE) + 
  labs(title = "PCA of TRA.AF",
       x = "PC1",
       y = "PC2",
       shape = "Spike Specific")
```
