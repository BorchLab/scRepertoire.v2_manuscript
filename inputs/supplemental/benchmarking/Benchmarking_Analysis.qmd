...
for (size in dataset_sizes) {
  for (i in seq_len(10)) {
    ...
    # Sample exactly `size` barcodes out of that
    sampled_pairs <- lookup %>%
      slice_sample(n = size)

    # Pull all rows from master.TCR 
...
## Benchmark Loop with Checkpointing
for (size in dataset_sizes) {
  for (rep in 1:10) {
    ds_dir <- file.path("datasets", size, paste0("replicate", rep))
    for (pipe in names(processors)) {
      
      # Check if this combo is already in the result table
      ...
      # Save after every benchmark to preserve progress
      fwrite(
        benchmark_results,
        benchmark_file
      )
